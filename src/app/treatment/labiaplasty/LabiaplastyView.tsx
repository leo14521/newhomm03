...
import { getLabiaplastyPageContent } from "@/i18n/labiaplastyPageContent";
import { useLocale } from "@/i18n/LocaleProvider";

export default function LabiaplastyView() {
  const { locale } = useLocale();
  const c = getLabiaplastyPageContent(locale);
  ...
  return (
    <main className="min-h-screen pt-[var(--header-height)] lg:pt-[var(--header-h)]">
      ...
      <section className="section-block relative bg-[var(--bg-card)]">
        <div className="mx-auto max-w-[800px]">
          ...
          <div className="mt-10 rounded-lg border border-[var(--border-page)] bg-[var(--bg-page)] p-6">
            <h4 className="font-[family-name:var(--font-kr-heading)] text-[17px] font-medium text-[var(--text-primary)]">
              {c.idealBoxTitle}
            </h4>
            <p className="mt-2 text-[14px] text-[var(--text-secondary)]">{c.idealBoxText}</p>
          </div>
        </div>
      </section>
      ...
      <section className="section-block relative bg-[var(--bg-card)]">
        <div className="mx-auto max-w-[800px]">
          ...
          <div className="mt-8 space-y-8">
            {c.s5Cards.map((card, i) => (
              <div key={i} className="rounded-lg border border-[var(--border-page)] bg-[var(--bg-page)] p-6">
                <h3 className="font-[family-name:var(--font-kr-heading)] text-[18px] font-medium text-[var(--text-primary)]">
                  {card.title}
                </h3>
                <p className="mt-4 text-[15px] leading-[1.8] text-[var(--text-secondary)]">{card.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section-block relative bg-[var(--bg-page)]">
        <div className="mx-auto max-w-[800px]">
          <div className="sec-head text-center">
            <span className="sec-label block">{c.s8Label}</span>
            <h2 className="sec-title mt-3 text-[clamp(24px,3.5vw,32px)]">{c.s8Title}</h2>
          </div>
